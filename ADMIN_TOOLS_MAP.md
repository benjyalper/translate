...
- **Per-part copy (`skSplitParts`):** segments that are bullet-delimited (`A • B • C`) — which in Starling are wrapped in numbered tag pairs `①…① ②…②` — render an extra **small Copy button per part**, each with a yellow index badge (1..N matching the tag number) + the English part as reference. Copy a part → paste it **between its tags** in Starling so the tag objects are never touched. The whole-segment button becomes **Copy all**. Applies in both the inline tool and the downloaded .html.
