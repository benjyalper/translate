...
| v | Change |
|---|---|
| — | *(panel-only)* **🧠 Semantic-accuracy pass II — multi-variant contextual memory, confidence tiers, correct QA order, and a pure testable core.** Finishes the first accuracy pass. Pure pipeline logic is extracted into **`pipeline-core.js`** (no DOM/chrome — loaded before `panel.js`, exposed as `PC`, unit-tested in `tests/core.test.js`). **(1) Multi-variant Consistency memory:** an entry now holds several contextual variants per English source (`Save\|button → שמירה` *and* `Save\|instruction → שמור/שמרי` coexist; writing one no longer overwrites the other). Legacy single-entry data reads unchanged; export/import/search/delete/backup are variant-aware; every write path (incl. the individual **✍ Write**, which previously dropped context) records `key`/`context`, and imports/corpus/manual adds store *context-unknown* rather than inventing context. **(2) Explicit context confidence** HIGH/MEDIUM/LOW/UNKNOWN from exact key, UI role (key suffix), namespace and note — two empty contexts are UNKNOWN (never auto-compatible), a role conflict is LOW, role-aware tie-break picks the right variant. Short sources (≤4 words) auto-apply only at HIGH (else review-only suggestion); long sentences apply at MEDIUM+; legacy/unknown capped at LOW. **(3) Context-aware dedupe** (`PC.planDedupe`) — short strings align only within a HIGH-confidence context; two empty-context "Save" strings are never forced together. **(4) Fixed processing order:** memory → Auto-fix → **reviewer** → validators, so the independent QA reviewer inspects the *final* semantic candidate and no semantic search-and-replace runs after it; the placeholder guard runs last on the reviewer's text. **(5) Reviewer parity:** the reviewer now also gets `fullSource`, the compact task context, and the **same cached screenshot** the translator saw (per-run cache, base64 never persisted or logged). **(6) Compact task-wide context** (`PC.buildTaskContext`) — a capped, relevance-filtered glossary of locked/harvested/high-confidence-memory terms that actually occur in the task, prepended to every batch and the reviewer (never invents Hebrew, never dumps the whole term base). **(7) Applicability + semantic risk** (`PC.classifyTerms`/`segRisk`) — a single word overlapped by a longer term (`Due` in `due to`) is dropped from the prompt; sense/POS-risky terms are marked; **`tbCheck` no longer raises a definite deviation** for those (softened to **🏷 term?** uncertain, or suppressed) — still non-mutating, locked terms still strict. **(8) Diagnostics** (`scDebug(true)`) now log the chosen memory variant + confidence + reasons + action, term applicability, risk, actual screenshot attachment, and task-context sizes — never keys/tokens/cookies/base64/URLs. Tests: **40 behavioural** (`tests/core.test.js`, real logic) + **38 structural/order** (`tests/pipeline.test.js`) = 78, all green; `node --check` on all scripts OK. Docs updated (`EDITING-PIPELINE.md`, `USER-MANUAL.md`). |
| — | *(panel-only)* **🎯 Translation-accuracy pass — terminology guides, it doesn't override.** A batch of changes so GPT makes fewer semantic/terminology/grammar/consistency errors. **(1) Term definitions to GPT:** `tbHintsFor` now sends `{en,he,pos,definition}` (the Starling definition was harvested but unused), and the `terms` prompt bullet was rewritten — a term applies **only** when the occurrence's sense *and* POS match its definition, a surface word-match is never proof, with baked-in examples (*Due→לתשלום* must not fire on *"due to"*=עקב; *Application→הגשת בקשה* ≠ *"verification application"*→בקשת האימות; noun *Highlight* ≠ the verb). **(2) Multi-word priority + relevance cap** in `tbHintsFor` (*"due to"* outranks *"Due"*; ≤12 hints). **(3) Priority preamble** at the top of the system prompt (meaning → context → locked → preferred terms → natural Hebrew → style). **(4) Deterministic placeholder guard** `phCheck` — a red **⚠ placeholder guard** badge when a `{x}`/`%s`/`<tag>`/①②③ token is dropped/added/altered; corruption never ships. **(5) Context-aware memory & dedupe:** for short (≤3-word) sources, a differing memory becomes a review *suggestion* (not an overwrite) and same-source alignment only fires within a shared context (key module + note); long sentences unchanged; new memory entries store `key`/`ctx`, legacy entries still work. **(6) Independent GPT QA** (Settings toggle, off by default, ~2× cost): a second reviewer model returns structured `OK`/`FIX`+type, applies a fix only if placeholders stay intact, and on failure keeps the original marked **🔎 QA n/a**. **(7) Screenshots as context** (Settings toggle, off, beta): a segment's Starling UI screenshot is fetched in-tab (credentialed, base64-inlined) and sent to a vision model as extra context, graceful text-only fallback. **(8) Diagnostics** `scDebug(true)` logs the evidence per segment (never secrets). Term enforcement stays **flag-only** — no stage does EN→HE substitution, so a correct contextual translation can't be surface-matched away. `node --check` OK; 23-case regression suite in `tests/pipeline.test.js` green. Docs updated (`EDITING-PIPELINE.md`, `USER-MANUAL.md`). |
| — | *(panel-only)* **⚖ Drift / consistency cards no longer look greyed-out.** A review card is dimmed (`.rc.unchanged`, opacity .5) when GPT's suggestion equals the text already in the segment — but a **⚖ drift** or **⚖ consistency** flag is *batch-level*: both competing renderings can already be present in the task, so every drifting row is "unchanged" yet still fully actionable (its **use ‹rendering›** / **keep current** buttons work). Those cards were dimmed like nothing-to-do rows, reading as inert. `renderReview` now tags a flagged card `has-drift` (`p.icDrift` or `p.consist`) and `.rc.unchanged.has-drift { opacity: 1 }` restores full strength, so drift cards you're meant to act on read clearly while ordinary unchanged rows stay dimmed. `node --check` OK |
| — | *(panel-only)* **⚖ Coverage preview under the ✨ Run button — see which stored terms will guide the run before spending the GPT call.** After a harvest (DOM or XLIFF), a new `runCoverage()` line under the Run button reports, against the harvested sources, exactly what the prompt will carry: **🔒 N locked** (mandatory, in the system prompt via `lockText`), **🏷 N term hints** (soft per-segment `terms` field via `tbHintsFor` — shows *"term base off"* when the 🏷 toggle is off), and **📖 N glossary** (advisory Style-Brain glossary in the system prompt), plus *"covering X/total segment(s)"* — the count of segments carrying ≥1 locked or term-base match (boundary-aware `lockSrcHas`, the same matcher the prompt uses). When nothing matches it prompts you to ⬇ grab a term base or add 🔒 locked terms. Re-computed on every harvest and kept in sync when terms change (called from `tbRefresh`/`lockRefresh`), so grabbing a term base or toggling the term base updates the number live. `node --check` OK |
| — | *(panel-only)* **⚖ In-task consistency: a self-consistency prompt rule + a GPT "Sweep drift" pass.** The existing safeguards only cover *identical-source* alignment and *defined-label* drift (`consistCheck`) — neither catches GPT rendering the same free verb/collocation two ways across differently-worded segments (e.g. "take (time)" → אורך in one, לוקח in another). Two additions: **(A)** an *INTERNAL CONSISTENCY* bullet in the TikTok style guide tells GPT to render a recurring word/verb/collocation with one consistent Hebrew choice across the batch (free prevention, every Run). **(B)** a **🧹 Sweep drift** button runs one GPT pass over the whole task's source→target pairs (`icSweep` → `icSweepGpt`) and flags genuine synonym drift as **groups** — for each drifting term (e.g. *information → מידע vs פרטים*) it returns the competing Hebrew **renderings** and, for every segment using that term, a full grammatically-correct rewrite under **each** rendering. Surfaced as a review-only **⚖ drift** badge + **⚖ Drift** filter; each flagged row shows one **“use ‹rendering›”** button per option **plus a “keep current”** button, and clicking a rendering **standardizes every segment in the task that uses that term** on the chosen rendering (so *you* pick the direction — GPT doesn't force a winner) while **keep current** dismisses the flag and leaves every segment's harvested wording untouched. Never auto-applied; grammar/sense-required differences are excluded. `node --check` OK (live harness run blocked — the files sit outside the project folder, so both browsers refused the local `file://`). |
| — | *(panel-only)* **📦 Corpus now harvests Document-editor tasks too — and a 📅 "created on/after" date filter.** The corpus builder read every Submitted task via `getSourceTextListWithTargetText`, which **1002s on doc tasks**, so they were counted as failures and never learned from (the "24 failing" tasks). `cbFetchTasks` now takes the task objects (with `taskType`) and, for a Document-editor task (`taskType 1`, or any task the string API 1002s on), reads **`/api/editor/getDocTaskDetailOnline?docTaskId=<id>`** — sending the `task-id` / `task-type: 1` / `starling-origin` headers the editor itself sends (without them the endpoint 1002s) — then folds `data.segmentInfo.segments` (per-segment `Status`, `Source.Text`/`Target.Text`) into the same pipeline: **`Status===3` confirmed only**, non-empty, keyed on `SegmentID`. String tasks keep the plural/`textExtra` lane; doc tasks have none. Verified live on doc task `434681596930`: string API `1002` → fallback → detail `1000` → **10/10 confirmed pairs**. Also new: a **📅 Created on/after** date input (`cb-since`) — `cbFetchMyTasks` now returns each task's `createTime` (unix seconds) and filters to tasks created on/after the chosen local day, applied to **⟳ Update**, **📦 Build**, and the "new tasks" badge; **Clear** resets it. `node --check` OK |
| v40 | **↩ Write now confirms Document-editor writes from the editor itself (the *right* fix for the false "COULDN'T verify" warning).** `writeAll` verifies by re-reading the task via `apiTask()` — the string content API that **1002s on every `/doc/editor` task** — so `byRank` was always null for doc tasks and every write fell into the blanket `via:'unverified'` branch, warning *"Wrote N via the editor but COULDN'T verify… (it may not have saved)"* even when it saved. (The earlier panel-side store rescue read `taskDetail…Target.Text` 450 ms later, which can still lag the editor's commit.) Live inspection of doc task `439018851842` confirmed the doc editor **commits typed text on blur** — its store segment `Target.Text` mirrors the `.cat-content__target` cell exactly — unlike the string editor's controlled input, which can accept text then silently revert. So `writeAll` now detects a doc task (`/\/doc\/editor\//`) and, when the API read fails, **trusts the DOM check `writeOne` already did** (`tgtSame(after, text)` — read from the cell right after typing, while the row was guaranteed visible), tagging a stuck write `via:'dom'` instead of `unverified`. A cell that didn't take the text still warns. String-editor tasks keep the strict behaviour (a failed API read there = genuinely unknown → `unverified`). CS v39→v40; `node --check` OK. **Ctrl+R the Starling tab after reloading the extension.** |
| — | *(panel-only)* **✋ Paste-by-hand parts now split on the real tag boundaries, not a bullet inside a tag.** A tagged segment like `O-1-0• Heading:C-1-0 body…` (a bolded "• Heading:" wrapped in one tag pair, then trailing text) was split by `splitParts` on the `•` **first**, before `splitTagRuns` — so the bullet, which sits *inside* the `O-1-0…C-1-0` pair, split there and produced garbage parts: **part 1 = the orphaned `O-1-0` token (strips to empty), part 2 = the entire rest of the sentence** (heading + body mashed together, `C-1-0` buried in the middle). `renderReview` now runs `splitTagRuns(p.next)` **first** and only falls back to the bullet splitter when there are **no** O-/C- tokens (a pure bullet list) — so the copy parts are the true runs *between* the chips (`• Heading:` │ `body…`), each pasteable between its matching `①…①`. The tag-run branch also now shows the aligned **source reference** per part (source runs pair 1:1 by tag structure). Verified on the doc-task rows `You've checked your ID's expiration date…` / `Your photo is clear…`: buggy `["O-1-0", "<whole rest>"]` → correct 2 runs each; `node --check` OK |
| — | *(panel-only)* **↩ Write now verifies Document-editor writes (fixes the false "COULDN'T verify" warning).** After a write, the extension re-reads the task via Starling's `getSourceTextListWithTargetText` to confirm each segment landed — but that API 1002s on `/doc/editor` tasks, so every doc-editor write came back `via:'unverified'` and reported *"Wrote N via the editor but COULDN'T verify against Starling's server (it may not have saved)"* even when it saved fine. `doWrite` now adds a **doc-store rescue**: for any still-`unverified` results it waits 450 ms (let the editor commit), reads the open doc editor's redux store via `hvDocRows(null)`, builds a folded-`source → [folded targets]` map, and for each written segment fold-matches (`wbFold`) its written text against the store's target for that source — a match reclassifies it `via:'store'` ("confirmed in the doc editor"). Same store as ⬇ Grab / 📚 Harvest; String-editor writes untouched (they still verify server-side). A genuine non-match stays unverified and still warns. `node --check` OK |
| — | *(panel-only)* **📚 Learn-from-task Harvest now works in the Document editor (`/doc/editor`).** The string content API 1002s on doc tasks, so `hvHarvest`'s `wbCall('API_TASK')` failed with *"Harvest failed: status_code 1002"*. Added `hvDocRows(id)`: when API_TASK fails, it `executeScript {world:'MAIN'}` on the active tab, walks up from a `.cat-content*` node to `memoizedProps.docEditor.taskDetail`, and reads every segment from `taskDetail.segmentInfo.segments` — `Source.Text`/`Target.Text` → source/target, `Status===3` → proofread-confirmed — returning API_TASK-shaped rows. Same store as ⬇ Grab; falls back only on doc tasks, String-editor harvest untouched. Verified live on doc task `413346827522`: **61 confirmed pairs** (65 segments) fed to memory/distill; `node --check` OK |
| — | *(panel-only, no CS bump)* **🏷 Term base — harvest Starling's own inline term references.** Starling underlines source words that carry an approved term entry (`span.highlight-text-term`); the hover popover gives **EN→HE + part-of-speech + a DNT/Brand flag**. New **🏷 Term base** card: **⬇ Grab term base** reads them straight from the open task via `executeScript {world:'MAIN'}` (`tbGrab`) — the popover data is preloaded on each span's React vnode, so it's read **without hovering** (verified: `content→תוכן` Noun, `TikTok→TikTok` DNT, `note→הערה`). Two tiers keyed on the DNT flag: **DNT/brand** terms route into **🔒 Locked terms** as keep-as-is (strict, `lockCheck`-enforced); **translatable** terms are stored in a new `TB` (`termBase`) store as **soft hints** — `tbHintsFor` injects only the terms whose EN appears in each segment into the GPT prompt (new `terms` item field + sysPrompt bullet) as the *preferred* rendering that may **bend when grammar/context requires** (a Noun used as a verb → `הדגש/י`; `"due to"` → `עקב`). `tbCheck` adds a review-only **🏷 term** badge + filter when a term's approved HE is missing — never rewrites. A **toggle** (`TB.enabled`) turns the whole thing off for harvest/write; export/import/password-gated clear. Scraper verified live; `node --check` OK |
| — | *(panel-only)* **🏷 Grab now sweeps the WHOLE task, not just the visible rows.** Starling's segment list is a **virtualized** custom scroller (only on-screen rows exist in the DOM), so a single-snapshot grab captured only the terms currently scrolled into view (e.g. *"2 terms"* when the visible rows were term-less fan-nicknames). `tbGrab`'s injected `world:'MAIN'` function now **picks the scroller** (scrollable ancestor of a term span, else the tallest scroll area) and **walks it top→bottom** — setting `scrollTop` + dispatching `scroll`, parsing the term spans that mount at each step into a dedupe map — then **restores the original scroll position**. Bottom detected when `scrollTop` stops advancing (`stuck≥2`); `guard<200`. Verified live on task `435504519170`: snapshot=2 → full sweep=**13** (surfaced `League·Subscribe·Share·Save·Tap` the snapshot missed); scroll restored; `node --check` OK |
| — | *(panel-only)* **🏷 Grab now works in the Document editor (`/doc/editor`), not just the String editor.** Doc tasks use different term markup (`span.cat-content__term`, no preloaded vnode) and the string content API 1002s on them — so grab found nothing. `tbGrab`'s injected `world:'MAIN'` function now detects a doc task by walking up from a `.cat-content*` node to the connected component whose `memoizedProps.docEditor.taskDetail` holds the redux store, then reads **every** segment's `termMap` from `docEditor.taskDetail.segmentInfo.segments` — `langItemMap.{source,target}.content` → EN/HE, `tags[].tag==='DNT'` (or description `"Brand name"`) → DNT lock, numeric `partOfSpeech` → POS (1 Noun · 2 Verb · 3 Adj · 4 Adv · 6 Proper noun). Whole task in **one read, no scroll, no hover** (the store isn't virtualized); falls through to the String-editor scroll-walk when it's not a doc task. Verified live on doc task `421379622658`: **26 terms** (TikTok DNT + 25 translatable — `report→דיווח` Verb, `content→תוכן`, `tap→הקשה`…); `node --check` OK |
...
| — | *(panel-only, no CS bump)* **🧹 Consolidate review now PERSISTS too.** The pending consolidate proposal (`CLS.result`) was in-memory only — closing the panel mid-review lost the whole (paid-for) GPT pass. Now `clsSave`/`clsLoad` (storage key `consolidateReview`) persist it on produce, on every ✕-drop, and at Replace/Discard (which clear it); init restores it via `clsLoad` → `clsRender` and auto-opens the 🧹 Consolidate section. Mirrors the `confSave`/`bpSave` pattern. Only the un-applied proposal is stored. |
| — | *(panel-only, no CS bump)* **🧹 Consolidate rules.** A Style-Brain-card tool (`clsRun`) that runs one GPT pass over your **existing** rules **per category** — merging duplicates, dropping near-repeats and tightening wording **without losing coverage** — so a brain grown to thousands of rules stays tight enough for GPT to actually follow (not skim) and cheap to send every batch. `brainText()` dumps *every* rule into the prompt verbatim with no cap, so a bloated brain both costs a fortune and suffers attention-dilution; this keeps it lean. Per-category consolidation (`clsConsolidateList`, chunked at 100 + a cross-chunk merge pass, global `wbNorm` dedupe) → a review list where you can ✕-drop any bad merge, with a "what changed" notes panel, then **✅ Replace** (`clsApply`, replaces `BRAIN.rules` only — glossary/locked/memory/auto-fix untouched). Auto-downloads a full backup first, so it's reversible. Verified: grouping/chunk/merge/dedupe 5/5 |
| — | *(panel-only, no CS bump)* **Conflict tickets + review proposal now PERSIST.** The orange ⚠ Conflicts (`CONF`) and any pending 🧠 Style-Brain review proposal (`brainProposal` — e.g. a corpus-learn run's findings) were in-memory only, so closing the side panel or reloading the extension lost anything un-adjudicated. Now `confSave`/`confLoad` (storage key `conflicts`) persist tickets on every add/resolve, and `bpSave`/`bpLoad` (key `brainReview`) persist the proposal via `brainRenderReview` + at both clears; init restores both. You can quit mid-adjudication and pick up exactly where you left off. |
| — | *(panel-only, no CS bump)* **📚 Learn from the whole corpus.** A new Corpus-card lane (`clRun`) that GPT-distills **every pair in the built corpus** into rules / terms / expressions / exceptions + conflicts, then merges via the existing 🧠 Style-Brain review flow. Two phases keep it careful and cheap: **(1) extract** from the pairs alone (`clSysExtract`), read **newest-submitted first** via a `getMyTasks` date join (`clFetchDates`/`clVariantDateMs`, id-order fallback) with an **evolution timeline** for contested sources (`clGatherPairs`) so it learns the *correction*, not both; **(2) reconcile** the deduped candidates against the current brain in one bounded pass (`clSysReconcile`) — already-covered rules are dropped, contradictions become **⚠ conflicts**. Glossary clashes still route to orange tickets via `brainMerge`. Auto-downloads a full brain **backup before merge**; nothing changes until you click Merge (or Discard to keep the brain as-is). “All sources” vs contested-only toggle. Verified: recency canonical-flip + timelines 5/5, gather 9002/360 on the real corpus |
