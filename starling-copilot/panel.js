...
// MEASURE = the per-task "measurement mode" toggle → unitFix also forces m→מ' , s→שנ'.
let MEASURE = false;
function polish(src, out) { return PC.numBidiFix(PC.unitFix(mirrorEdges(src, matchTrailingNL(src, matchTrailingPeriod(src, fixBold(src, fixAmounts(src, fixBrands(fixSpacing(out))))))), MEASURE)); }
