...
function polish(src, out) { return PC.numBidiFix(PC.unitFix(mirrorEdges(src, matchTrailingNL(src, matchTrailingPeriod(src, fixBold(src, fixAmounts(src, fixBrands(fixSpacing(out))))))))); }
...
const STYLE_GUIDE =
  ...
  '- UNITS — MATCH THE SOURCE FORM, and be CONSISTENT (the SAME unit renders the SAME way throughout a task): an ABBREVIATED source unit becomes the Hebrew ABBREVIATION, a SPELLED-OUT source unit becomes the Hebrew WORD, always with ONE space after the number (keep the digit). Abbreviations → s→"שנ\'" (seconds), min→"דק\'" (minutes), h/hr→"שע\'" (hours), m→"מ\'" (METERS), km→"ק"מ", cm→"ס"מ", mm→"מ"מ", kg→"ק"ג", mg→"מ"ג", ml→"מ"ל", cc→"סמ"ק", km/h→"קמ"ש", g→"גרם", l/L→"ליטר", V→"וולט". Spelled out → seconds→"שניות", minutes→"דקות", hours→"שעות", days→"ימים", meter/meters→"מטר", kilometer→"קילומטר", kilogram→"קילוגרם", gram→"גרם", liter→"ליטר". Examples: "20s"→"20 שנ\'", "20 seconds"→"20 שניות", "a 5m cable"→"כבל 5 מ\'", "5 meters"→"5 מטר", "450kg"→"450 ק"ג", "12V"→"12 וולט". DISAMBIGUATE by context: "m" is METERS (מ\') in a dimension context, MINUTES (דק\') in a time context, and MILLION in "5m views/likes" (keep the meaning — not a unit); a MODEL/name like "iPhone 5s" stays as-is. STAY LATIN exactly as source: data sizes (KB/MB/GB/TB), "%", resolutions (1080p/4K), "60fps", "Mbps", and a network generation ("5G" stays "5G").\n' +
...
const FIX_SEED = [
  ...
  { from: 'משוך/י', to: 'משוך/משכי' }, { from: 'משכו', to: 'משוך/משכי' },
  { from: 'הזז/י', to: 'הזז/הזיזי' }, { from: 'הזיזו', to: 'הזז/הזיזי' },
...
function ycIsChanged(c) { return !!(c.proposal || '').trim() && !sameRender(c.proposal, c.tgt || ''); }
function setYcFilter(m) {
  ycFilter = m;
  ['changed', 'all', 'manual'].forEach((k) => { const b = $('yc-view-' + k); if (b) b.classList.toggle('active', k === m); });
  ycRender();
}
...
function ycRender() {
  ...
  const shown = YC.cards.map((c, i) => ({ c, i })).filter(({ c }) =>
    ycFilter === 'all' ? true :
    ycFilter === 'manual' ? c.tagged :        // ✋ paste-by-hand (tagged) segments
    ycIsChanged(c));                          // 'changed' (default)
  ...
  const ok = YC.cards.filter((c) => c.status_ui === 'written').length;
  const changedN = YC.cards.filter(ycIsChanged).length;
  const taggedN = YC.cards.filter((c) => c.tagged).length;
  if ($('yc-count')) $('yc-count').textContent = `${shown.length} shown · ${changedN} changed · ✋ ${taggedN} by hand`
    + (YC.write ? ` · ${ok}/${YC.cards.length} written` : '');
}
...
async function init() {
  ...
  if ($('yc-appr-all')) $('yc-appr-all').addEventListener('click', () => { YC.cards.forEach((c) => { if ((c.proposal || '').trim()) c.approved = true; }); ycRender(); });
  if ($('yc-appr-none')) $('yc-appr-none').addEventListener('click', () => { YC.cards.forEach((c) => { c.approved = false; }); ycRender(); });
  if ($('yc-view-changed')) $('yc-view-changed').addEventListener('click', () => setYcFilter('changed'));
  if ($('yc-view-all')) $('yc-view-all').addEventListener('click', () => setYcFilter('all'));
  if ($('yc-view-manual')) $('yc-view-manual').addEventListener('click', () => setYcFilter('manual'));
