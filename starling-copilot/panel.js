...
const STYLE_GUIDE =
  ...
  '- METRIC UNITS: a metric unit stuck to (or following) a number is written as its Hebrew abbreviation/word with ONE space (keep the digit): "kg" → "ק\\"ג", "km" → "ק\\"מ", "cm" → "ס\\"מ", "mm" → "מ\\"מ", "mg" → "מ\\"ג", "ml" → "מ\\"ל", "cc" → "סמ\\"ק", "km/h" → "קמ\\"ש", grams "g" → "גרם", liters "l"/"L" → "ליטר", volts "V" → "וולט". E.g. "450kg" → "450 ק\\"ג", "5 cm" → "5 ס\\"מ", "200g" → "200 גרם", "2L" → "2 ליטר", "12V" → "12 וולט". The letter "m" after a number is MINUTES in a time context ("5m" → "5 דקות") but METERS in a physical/dimension context ("a 5m cable" → "כבל באורך 5 מטר") — decide by context; and "5m views/likes" means MILLION, keep the meaning. Always a space between the number and the unit. But a network generation stays Latin ("5G" stays "5G", NOT grams), and data sizes (KB/MB/GB/TB), "%", resolutions (1080p/4K) stay Latin as above.\n' +
